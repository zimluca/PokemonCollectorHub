...
export class DatabaseStorage implements IStorage {
  ...
  async loginUser(username: string, password: string): Promise<User | null> {
    return null; // MemStorage not used in production
  }

  async registerUser(userData: any): Promise<User> {
    throw new Error("MemStorage not used in production");
  }
...
export class DatabaseStorage implements IStorage {
  ...
  async syncPokemonCards(): Promise<void> {
    ...
    try {
      // Get or create "Single Cards" product type
      let cardType = await db
        .select()
        .from(productTypes)
        .where(eq(productTypes.name, "Single Cards"))
        .limit(1);
      ...
      for (const setId of popularSets.slice(0, 3)) { // Limit to first 3 sets
        console.log(`Syncing set: ${setId}`);
        
        // Get all cards from the set
        const cards = await pokemonAPI.getAllCardsFromSet(setId);
        
        // Create or update collection for this set
        ...
        let collection = await db
          .select()
          .from(collections)
          .where(eq(collections.name, setInfo.name))
          .limit(1);

        if (collection.length === 0) {
          [collection[0]] = await db
            .insert(collections)
            .values({
              name: setInfo.name,
              nameIt: setInfo.name,
              description: `${setInfo.series} series - ${setInfo.printedTotal} cards`,
              descriptionIt: `Serie ${setInfo.series} - ${setInfo.printedTotal} carte`,
              imageUrl: setInfo.images.logo,
              releaseDate: new Date(setInfo.releaseDate),
            })
            .returning();
        }
        ...
        for (let i = 0; i < cards.length; i += 25) {
          ...
          for (const card of batch) {
            const productData = {
              ...
              name: card.name,
              nameIt: card.name,
              description: `${card.supertype}${card.subtypes?.length ? ` - ${card.subtypes.join(', ')}` : ''}`,
              descriptionIt: `${card.supertype}${card.subtypes?.length ? ` - ${card.subtypes.join(', ')}` : ''}`,
              collectionId: collection[0].id,
              productTypeId: cardType[0].id,
        ...
        console.log(`Completed syncing ${cards.length} cards from set ${setId}`);
        
        // Delay between sets to be respectful to the API
        await new Promise(resolve => setTimeout(resolve, 200));
      }
      
      console.log('Pokemon cards synchronization completed!');
    } catch (error) {
      console.error('Error during Pokemon cards sync:', error);
      throw error;
    }
  }
}
